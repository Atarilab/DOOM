...
from utils.helpers import ObservationHistoryStorage, reorder_robot_states
...
class RLLocomotionVelocityController(ControllerBase):
    ...
    def register_observations(self):
        ...
        self.obs_manager.register('velocity_commands', ObsTerm(velocity_commands))
    ...
    def compute_torques(self, state, desired_goal):
        ...
        joint_pos_targets = joint_pos_targets[self.actions_isaac_to_unitree_mapping].detach().numpy() # re-ordering
     
        # torques = self.Kp * (joint_pos_targets - state['joint_pos']) - self.Kd * state['joint_vel']
