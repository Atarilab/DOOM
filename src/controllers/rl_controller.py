...
from state_manager.observations import (
    joint_pos_rel, joint_vel, lin_vel_b, ang_vel_b, 
    last_action, projected_gravity_b, velocity_commands, feet_pos
)
...
class BaseRLLocomotionController(ControllerBase):
    ...
    def _load_policy_model(self, configs: Dict[str, Any]):
        ...
        self.action_scale = torch.tensor(
            configs['controller_config']['action_scale'], 
            dtype=torch.float32
        )
    ...
    def _initialize_controller_parameters(self, configs: Dict[str, Any]):
        ...
        self.default_joint_pos = torch.tensor(
            controller_config["ISAAC_LAB_DEFAULT_JOINT_POS"], 
            dtype=torch.float32
        )
    ...
    def _init_processing_threads(self):
        ...
        self.policy_inference_thread = threading.Thread(
            target=self._run_policy_inference, 
            daemon=True
        )
    ...
    def compute_torques(self, state, desired_goal):
        ...
        try:
            ...
            ).cpu().numpy()[self.actions_isaac_to_unitree_mapping]
...
class RLLocomotionVelocityController(BaseRLLocomotionController):
    ...
    def register_observations(self):
        ...
        self.obs_manager.register('velocity_commands', ObsTerm(
            velocity_commands, 
        ...
        self.obs_manager.register('joint_vel', ObsTerm(
            joint_vel,
            params={'mapping': self.joint_obs_unitree_to_isaac_mapping}
        ))
        self.obs_manager.register('last_action', ObsTerm(
            last_action, 
            params={"last_action": lambda: self.raw_action}
        ))
        self.obs_manager.register('feet_pos', ObsTerm(
            feet_pos, 
