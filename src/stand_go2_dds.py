...
class RobotController:
    ...
    def __init__(
        self, mode_manager: ModeManager, logger: Optional[logging.Logger] = None
    ):
    ...
    async def run_control_loop(self, publisher: ChannelPublisher):
        ...
        while True:
            ...
            # Compute torques
            state = {"elapsed_time": asyncio.get_event_loop().time()}
...
async def main():
    ...
    parser.add_argument(
        "--log", type=str, required=True, help="Experiment name to log information"
    )
    ...
    try:
        ...
        if "sim" in args.task:
            ...
            ChannelFactoryInitialize(
                robot_interface_config["DOMAIN_ID"], robot_interface_config["INTERFACE"]
            )
        else:
            network_interface = os.environ.get("NETWORK_INTERFACE")
            logger.info(
                f"Initializing channel with network interface: {network_interface}"
            )
        ...
        mode_manager.register_mode("IDLE", {"default": IdleController()})
        ...
        mode_manager.register_mode("STANCE", {"STANCE": StanceController(robot_config)})
        ...
        await asyncio.gather(
            robot_controller.run_control_loop(publisher), app.run_async()
        )
