...
def object_state_handler(msg: Dict[str, List], logger: Optional[logging.Logger] = None):
    ...
    return {
        "object_pos_w": list(np.array(base_pos_w) + np.array([-0.7, 0.0, 0.0] )), # more negative values move the beginning of the step towards the robot, ie make the robot take the step on the box faster
